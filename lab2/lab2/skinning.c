...
// gcc skinning.c ../common/*.c ../common/Mac/MicroGlut.m -o skinning -framework OpenGL -framework Cocoa -I../common/Mac -I../common
...
void DeformCylinder()
{
    ...
    Point3D vertsTemp1[kMaxRow][kMaxCorners];
 	Point3D vertsTemp2[kMaxRow][kMaxCorners];
    
    vec4 bonePos;
//    bonePos = vec4(g_bones, 1);
	...
	for (row = 0; row < kMaxRow; row++)
	{
		for (corner = 0; corner < kMaxCorners; corner++)
		{
			...
			// corner traverserar "runt" cylindern
             ...
             vec3 v1 = VectorAdd(MultVec3(g_bones[0].rot, g_vertsOrg[row][corner]), g_bones[0].pos);
             vec3 v2 = MultVec3(Transpose(g_bones[0].rot),(VectorSub(v1, g_bones[0].pos)));
             v2 = VectorSub(v2,g_bones[1].pos);
             v2 = VectorAdd(MultVec3(g_bones[1].rot, v2), g_bones[1].pos);
             v2 = VectorAdd(MultVec3(g_bones[0].rot, v2), g_bones[0].pos);
             
             g_vertsRes[row][corner] = VectorAdd(ScalarMult(v1, (1-weight[row])), ScalarMult(v2, weight[row]));
